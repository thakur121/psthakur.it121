...
@Component({
  selector: 'app-driverdtls',
  templateUrl: './driverdtls.page.html',
  styleUrls: ['./driverdtls.page.scss'],
})
export class DriverdtlsPage implements OnInit {
	...
	emargencypopup:any=0;
  	...
  	constructor(private notiser:NotificationService,public navCtrl: NavController,private geolocation: Geolocation, public rest: RestService,public route: Router) 
      { 
            if(this.firstcall==0)
            {
                  this.firstcall=1;
      ...
      gotoaccptedtrip()
      {
            ...
            this.servicedata.push({"status":'8'});
            this.servicedata.push({"address": this.currentposition});
            this.servicedata.push({"role": '2'});
            this.servicedata.push({"payment_type":'1'});
            this.servicedata.push({"amount":this.RequestRide.charge});
